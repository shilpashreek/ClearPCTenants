...
public class Search_pageTest extends BaseClass {
	...
	Xls_Reader reader;
	String FilterDetailsSheet = "ApplyAllFilters";
	...
	public void ValidateSearchTipsMessageIsDispalyed() throws Exception {
		...
		Assert.assertTrue(search_tips.contains(ConfigUtils.getConfigData("searchTipsMessage")),
				"Search tips message is not displayed");
	...
	public Object[][] searchFilters() {
		...
		Object[][] filterDetails = reader.GetData(FilterDetailsSheet);
		return filterDetails;
	}

	// , dataProvider = "searchFilters"

	@Test(priority = 1, enabled = true, dataProvider = "searchFilters")
	public void performSearchWithinOrganisation(String FilterContent, String content_type, String locator) {
		...
		searchPage.clickOnSearchBtn();
		// searchPage.enterSearchText_CaptureFilterSuggestions(ConfigUtils.getConfigData("searchInput"),"Folders");
		// searchPage.captureSearchResults(FilterContent, locator);
		// searchPage.captureSearchResults();
		// String lcoatorValue = locator;
		// WebElement a = driver.findElement(By.xpath(lcoatorValue));

		/*
		 * searchPage.enterSearchText_CaptureFilterSuggestions(ConfigUtils.getConfigData
		 * ("searchInput"), FilterContent, driver.findElement(By.cssSelector(locator)));
		 */

		searchPage.searchResultsVerificationForAllContentTypes(ConfigUtils.getConfigData("searchInput"), FilterContent,
				locator);

		SeleniumUtility.waitTill_invisibility_of_Element(driver, 20, genericUtils.textLoadingSymbol());
		searchPage.getSearchResultsCount();
		boolean searchResultVerification = searchPage.verifySearchResults(content_type,
				ConfigUtils.getConfigData("searchInput"));
	...
	public void validateAutosuggetionsForAdvancedSearch() {
		...
		SeleniumUtility.waitTill_invisibility_of_Element(driver, 30, genericUtils.loadingSymbol());
		searchPage.clickOnSearchBtn();
		int AutoSuggestionsCount = searchPage.captureAutosuggestion(ConfigUtils.getConfigData("searchInput"),
				ConfigUtils.getConfigData("Autosuggestions_path"));
