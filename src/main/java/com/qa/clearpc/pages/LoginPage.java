...
public class LoginPage extends BaseClass {
	...
	@FindBy(name = "x-password")
	WebElement Password;
	@FindBy(id = "loginSubmit")
	WebElement loginBtn;
	@FindBy(id = "editUserName")
	WebElement edit_userName;
	@FindBy(id = "ctl00_pageContent_passwordEmail")
	WebElement OtpTextArea;
	@FindBy(css = ".promptBtn.submitbtn.MFAButtonSubmit")
	WebElement submitBtn;
	@FindBy(id = "Resend")
	WebElement resendBtn;
	@FindBy(css = ".marT5")
	WebElement passwordExp_InfoMessage;
	@FindBy(css = ".FL.newBackBtn")
	WebElement skipButton;
	@FindBy(className = "ResetPasswordSubmit")
	WebElement changePassword;
	@FindBy(css = ".name.FL")
	WebElement ActiveUserName;
	@FindBy(css = ".invalidCredientials.marL10")
	WebElement incorrrectSecurityCode;
	...
	public String login(String Username, String PassworD, String host, String username, String password,
			String Expected_Sender, String Expected_subject, String Filepath) throws Exception {
		String PageURL = null;
			...
			if (driver.getCurrentUrl().equalsIgnoreCase(ConfigUtils.getConfigData("Mfa_page_url"))) {
				try {
					...
					String mailBody = mailUtils.readTheLatestMail(host, username, password, Expected_Sender,
							Expected_subject, Filepath);
					String otp = mailUtils.getOtpFromMailBody(mailBody);
					// String otp = mailUtils.readTheLatestMail(host, username, password,
					// Expected_Sender, Expected_subject, Filepath); //existing
					enterOtp(otp); //
	...
	public HomePage enterOtp(String value) throws Exception, Exception {

		SeleniumUtility.Clear(driver, 10, OtpTextArea);
		SeleniumUtility.sendkeys(driver, 20, OtpTextArea, value); // value
		SeleniumUtility.Click(driver, 10, submitBtn);
		if (SeleniumUtility.elementDispalyed(incorrrectSecurityCode)
				&& SeleniumUtility.getAttribute(OtpTextArea, "class").contains("mfa_loginText appendError")) {
	...
	private void resendOTP() throws Exception, IOException {

		SeleniumUtility.Click(driver, 20, resendBtn);
		Thread.sleep(5000);
		String mailBody = mailUtils.readTheLatestMail(ConfigUtils.getConfigData("host"),
				...
				ConfigUtils.getConfigData("Sender"), ConfigUtils.getConfigData("Subject"), Constants.mail_Body_path);
		String otp = mailUtils.getOtpFromMailBody(mailBody);
		SeleniumUtility.Clear(driver, 10, OtpTextArea);
		SeleniumUtility.sendkeys(driver, 20, OtpTextArea, otp); // otp
		SeleniumUtility.Click(driver, 10, submitBtn);
